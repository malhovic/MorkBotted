...
def _pick_features(class_template: ClassTemplate, picks: dict[str, int]) -> list[ClassFeature]:
    chosen: list[ClassFeature] = []
    for category, count in picks.items():
        options = [feature for feature in class_template.features if feature.category == category]
        if not options:
            continue
        chosen.extend(random.sample(options, min(count, len(options))))
    return chosen
...
def generate_random_character(
    *,
    ...
    selected_features = _pick_features(class_template, rules["feature_picks"])
    ...
    return Character(
        ...
        silver=silver,
        equipment=equipment,
        notes=["Generated by MorkBotted scvmbirth."],
        selected_class_feature_ids=[feature.id for feature in selected_features if feature.id is not None],
